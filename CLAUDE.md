...
├── 棚1案A (MD40 + 奥行450)   下段。MD40 / G20-14R630 / カゴ2個・横置き
├── 棚2案A (MD40 + 奥行450)   上段。同上
├── 棚1案B (MD30 + 奥行300)   下段。MD30 / E20-1R630 / カゴ1個・縦置き
└── 棚2案B (MD30 + 奥行300)   上段。同上
```

棚柱案A・案B は同じ Y 位置に立つので、見比べるときはどちらかの `Visibility` を落とす。
...
スロット 0 として、`arm_pitch * arm_slot1` / `arm_slot2` で棚受の上面（＝棚板の下面）を
決める。棚板とカゴもこの式から派生する。スロットの原点が棚柱の下端かどうかは現物で要確認。

棚柱案B・案C の下端も同じスロット格子に乗せてある（`post_bottom_z - arm_pitch *
オフセット`）。案B のオフセット 60 は幅木（55）を越える範囲で最大。61 にすると下端が 46 に
なり幅木に当たる。案C は長さ 1200 なので、上段の棚受（上端 2003）を拾える範囲で最大の 21。
22 にすると上端が 1987 になり上段の棚受が乗らない。

最上段は窓枠の上面 2019 に合わせてある。窓枠の上面もドア枠の上面（2000）もスロットに
乗らないので、最寄りの 42 に寄せて棚板の上面が 2023（窓枠より 4 上）。両者の差は 19 で
ちょうど 1 ピッチなので、格子の原点を 4 下げると両方ぴったり乗る。
...
| 棚受の上下間隔 228（スロット 12 個）以上 | 722（38 個） |
...
| `arm_slot1` / `arm_slot2` | 4 / 42 | 棚受の位置（棚柱の下端から何ピッチ目か） |
