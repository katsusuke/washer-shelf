...
├── 棚1案A / 棚2案A / 棚3案A   MD40 / G20-15R630 / カゴ2個・横置き
...
| 棚板 | G20-15R630-4WV（630×450×20） | E20-1R630-WV（630×300×20） |
...
| 列 | 内容 |
|---|---|
| A / B / C | カテゴリ（製品ブロックではメーカー）/ 型番 / 備考 |
| E / F | パラメーター / 値（エイリアスは F 列に付く） |
| G | 出所（実測 / カタログ / 設計 / 推定 / 導出） |
| H / I / J | 必要数 / 入数 / 購入数（`=ceil(必要数 / 入数)`） |

上から順に、実測と配置（〜76 行）、検査（77 行〜）、構成（93 行〜）、製品（101 行〜）。
製品ブロックにはメーカー・型番・スペック・数量が入る。カゴだけ出所が実測（現物を測った）。
...
/ `b_basket_per_tier`）、棚柱の本数（`post_count`）が 93 行以降にあり、そこから算出する。

| 型番 | 必要数 | 入数 | 購入数 |
|---|---|---|---|
| SS-H06W / SS-H12W / SS-H18W | 2 | 2 | 1 |
| SS-MD40W（案A 棚受） | 6 | 2 | 3 |
| G20-15R630-4WV（案A 棚板） | 3 | 1 | 3 |
