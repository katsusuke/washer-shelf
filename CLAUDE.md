...
├── 棚柱案C (SS-H12W 1200)  長さ 1200。下端 597
...
なり幅木に当たる。案C は長さ 1200 なので、上段の棚受（上端 1794）を拾える範囲で最大の 32。
33 にすると上端が 1778 になり上段の棚受が乗らない。
...
| 棚受の上下間隔 228（スロット 12 個）以上 | 513（27 個） |
...
| `arm_slot1` / `arm_slot2` | 4 / 31 | 棚受の位置（棚柱の下端から何ピッチ目か） |
...
| `c_post_slot_offset` / `c_post_bottom_z` | 32 / 597 | 棚柱案C の下端（案A から何ピッチ下か） |
