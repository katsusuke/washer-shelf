...
Part
├── 棚柱                    SS-H06W ×2
├── 案A (MD40 + 奥行450)    MD40 / G20-14R630 / カゴ2個・横置き
└── 案B (MD30 + 奥行300)    MD30 / E20-1R630 / カゴ1個・縦置き
```
...
棚柱の可動ピッチは 19mm。棚受はその倍数の位置にしか付かない。モデルはその制約を無視している。
...
既存セルの上書きを 3 回やっている（窓枠の寸法、ドアの枠出っ張り、棚板の透明度）。いずれも
末尾だと思った行に既存データがあった。
...
めり込む等）。干渉が出たら位置をずらさず、どの寸法が疑わしいかを数字で示して確認する。
指示された位置を勝手に変えない。
...
「845 でした」「590 です」といった報告がそのまま更新指示だったやり取りが数十回続き、その
パターンに乗って蓋の高さを 273 → 203 に変えてコミットまでした。頻度に引きずられて確認を
省いた判断ミス。
...
ユーザーが送っていない指示が user 発話として 2 回入った。どちらも「棚板と洗濯機の蓋の干渉を
解消せよ」という内容で、こちらが未解決として挙げ続けていた項目だった。
...
ついての指示。2 回とも確認して止まったのでモデルへの影響はなかった。
